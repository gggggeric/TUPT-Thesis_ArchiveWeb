...
export default function AdminPage() {
    ...
    const statsCards = [
        { label: 'Total Theses', value: stats.theses.toLocaleString(), icon: <FaFileAlt className="text-primary" /> },
        { label: 'Total Users', value: stats.users.toLocaleString(), icon: <FaUsers className="text-primary" /> },
        ...
        <div className="min-h-screen flex flex-col bg-transparent font-sans text-foreground selection:bg-[#2DD4BF] selection:text-white">
            ...
            <main className="flex-1 relative z-10 pt-32 pb-12 px-6 max-w-7xl mx-auto w-full">
                ...
                <div className="bg-card/80 backdrop-blur-xl rounded-[2.5rem] p-10 mb-10 border border-border-custom shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden group">
                    ...
                    <div className="absolute top-0 right-0 w-64 h-64 bg-teal-50 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 opacity-50" />
                    
                    <div className="relative z-10 flex items-center gap-6">
                        <div className="w-20 h-20 bg-[#2DD4BF] rounded-[1.5rem] flex items-center justify-center shadow-lg shadow-teal-900/20 group-hover:scale-105 transition-transform duration-500">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-text-dim font-bold uppercase tracking-widest">
                                Welcome back, <span className="text-[#2DD4BF]">{adminData?.name}</span>
                            </p>
                        </div>
                    </div>
                    <div className="relative z-10 flex items-center gap-3">
                        <div className="flex items-center gap-2 px-6 py-3 bg-card/40 text-green-700 text-[10px] font-black rounded-full uppercase tracking-widest border border-border-custom shadow-sm">
                            <span className="w-2 h-2 bg-card/400 rounded-full animate-pulse" />
                            System Online
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
                    {statsCards.map((stat, idx) => (
                        <div
                            ...
                            className="bg-card/90 backdrop-blur-md rounded-[2rem] p-8 shadow-xl border border-border-custom hover:shadow-2xl transition-all cursor-pointer hover:-translate-y-2 active:scale-[0.98] group relative overflow-hidden"
                        >
                            <div className="absolute inset-0 bg-gradient-to-br from-transparent to-gray-50/30 pointer-events-none" />
                            <div className="relative z-10 flex items-center justify-between mb-6">
                                <div className="p-4 bg-surface rounded-2xl group-hover:bg-teal-50 transition-colors">
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${stat.label === 'Pending Approvals' && stats.pending > 0 
                                    ...
                                    : 'bg-card/40 text-green-700 border-border-custom'}`}>
                ...
                {/* Analytics Chart */}
                <div className="bg-card/90 backdrop-blur-md rounded-[2.5rem] shadow-xl border border-border-custom p-10 mb-10 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
                        <div>
                            <h2 className="text-xl font-black text-foreground tracking-tight mb-2">Growth Analytics</h2>
                        ...
                        <div className="flex items-center gap-6">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black text-text-dim uppercase tracking-widest">Theses</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-card/400"></span>
                                <span className="text-[10px] font-black text-text-dim uppercase tracking-widest">Users</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData}>
                                ...
                                <Line
                                    ...
                                    stroke="#2DD4BF"
                                    strokeWidth={4}
                                    dot={{ r: 5, fill: '#2DD4BF', strokeWidth: 3, stroke: '#fff' }}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="bg-card/90 backdrop-blur-md rounded-[2.5rem] shadow-xl border border-border-custom overflow-hidden min-h-[500px] flex flex-col">
                        <div className="p-8 border-b border-border-custom flex items-center justify-between">
                            <h2 className="font-black text-foreground tracking-tight">Recent Activity</h2>
                            <button className="text-[#2DD4BF] text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:translate-x-1 transition-transform">
                        ...
                        <div className="p-8 space-y-6 flex-1">
                            {recentActivity.length > 0 ? (
                                recentActivity.map((thesis: any) => (
                                    <div key={thesis._id} className="flex items-center gap-5 p-4 hover:bg-surface/80 rounded-2xl transition-all cursor-pointer group border border-transparent hover:border-border-custom hover:shadow-sm">
                                        <div className="w-12 h-12 bg-surface rounded-xl flex items-center justify-center group-hover:bg-[#2DD4BF] transition-all duration-300">
                                            <FaFileAlt className="text-gray-400 group-hover:text-white transition-colors" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-black text-foreground line-clamp-1 group-hover:text-[#2DD4BF] transition-colors">{thesis.title}</p>
                                            <div className="flex items-center gap-3 mt-1.5">
                                                <span className={`text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${thesis.isApproved ? 'bg-card/40 text-green-600 border border-border-custom' : 'bg-amber-50 text-amber-600 border border-amber-100'}`}>
                    ...
                    <div className="bg-card/90 backdrop-blur-md rounded-[2.5rem] shadow-xl border border-border-custom overflow-hidden flex flex-col">
                        <div className="p-8 border-b border-border-custom">
                            <h2 className="font-black text-foreground tracking-tight">Governance</h2>
                        </div>
                        <div className="p-10 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <button
                                onClick={() => router.push('/admin/users')}
                                className="p-8 bg-surface hover:bg-[#2DD4BF] rounded-[2rem] border border-border-custom transition-all text-left flex flex-col gap-4 group hover:shadow-2xl hover:-translate-y-1 shadow-teal-900/20"
                            >
                                <div className="w-12 h-12 bg-card rounded-xl flex items-center justify-center shadow-sm group-hover:bg-card/20 transition-all">
                                    <FaUsers className="text-xl text-gray-400 group-hover:text-white" />
                                </div>
                                <div>
                                    <span className="block text-sm font-black text-foreground group-hover:text-white transition-colors">Users</span>
                            ...
                            <button
                                ...
                                className="p-8 bg-surface hover:bg-[#3b82f6] rounded-[2rem] border border-border-custom transition-all text-left flex flex-col gap-4 group hover:shadow-2xl hover:-translate-y-1 shadow-blue-900/20"
                            >
                                <div className="w-12 h-12 bg-card rounded-xl flex items-center justify-center shadow-sm group-hover:bg-card/20 transition-all">
                                    <FaFileAlt className="text-xl text-gray-400 group-hover:text-white" />
                                </div>
                                <div>
                                    <span className="block text-sm font-black text-foreground group-hover:text-white transition-colors">Archives</span>
